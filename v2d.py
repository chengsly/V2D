...
allowed_by_model = {
    "linear": set(),
    "mlp":    {"n_neurons", "n_layers"},
    "tree":   {"min_samples_leaf", "max_depth"},
    "rf":     {"min_samples_leaf", "max_depth", "n_estimators"},
    "xgboost":{"max_depth", "n_estimators", "min_child_weight", "gamma",
               "subsample", "scale_pos_weight", "learning_rate"},
}
...
# --- Optional: forbid seeds for linear if that’s your policy ---
if model == "linear":
    if _is_set(getattr(args, "init_seed", None)):
        raise ValueError("--init_seed cannot be used with --model linear")
    if _is_set(getattr(args, "nbseed", None)):
        raise ValueError("--nbseed cannot be used with --model linear")

# --- Friendly echo (avoid else binding bugs) ---
if model == "linear":
